...
func TestGenerateKeys_differentKeys(t *testing.T) {
	keyShares, _, err := rsago.NewKey(keyTestSize, keyTestK, keyTestL, &rsago.KeyMetaArgs{})
	if err != nil {
...
func TestGenerateKeys_validRandom(t *testing.T) {
	...
	keyMetaArgs := &rsago.KeyMetaArgs{}

	keyShares, keyMeta, err := rsago.NewKey(keyTestSize, uint16(k), uint16(l), keyMetaArgs)
	if err != nil {
		t.Errorf(fmt.Sprintf("%v", err))
	}
	docHash := sha256.Sum256([]byte(keyTestMessage))

	docPKCS1, err := rsago.PrepareDocumentHash(keyMeta.PublicKey.Size(), keyTestHashType, docHash[:])
	if err != nil {
		t.Errorf(fmt.Sprintf("%v", err))
	}

	sigShares := make(rsago.SigShareList, l)
...
func TestGenerateKeys_validFixed(t *testing.T) {
	...
	keyShares, keyMeta, err := rsago.NewKey(keyTestFixedSize, uint16(k), uint16(l), keyMetaArgs)
	if err != nil {
		t.Errorf(fmt.Sprintf("%v", err))
	}
	docHash := sha256.Sum256([]byte(keyTestMessage))

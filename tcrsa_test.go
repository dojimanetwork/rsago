...
func Example() {
	...
	keyShares, keyMeta, err := rsago.NewKey(exampleSize, uint16(k), uint16(l), nil)
	if err != nil {
	...
	docPKCS1, err := rsago.PrepareDocumentHash(keyMeta.PublicKey.Size(), crypto.SHA256, docHash[:])
	if err != nil {
		panic(fmt.Sprintf("%v", err))
	}
...
func TestKeyShare_Sign_PssDoc(t *testing.T) {
	...
	keyShares, keyMeta, err := rsago.NewKey(bitSize, k, l, nil)
	assert.NoError(t, err)

	data := []byte("this is a test data 111")
	docHash := sha256.Sum256(data)
	docPSS, err := rsago.PreparePssDocumentHash(keyMeta.PublicKey.N.BitLen(), docHash[:], salt[:], nil)
	assert.NoError(t, err)
	sigShares := make(rsago.SigShareList, l)
